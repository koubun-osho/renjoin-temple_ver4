  ...
  return (
    <section className={`relative min-h-screen flex items-center justify-center overflow-hidden ${className}`}>
      ...
      <div className="relative z-10 text-center px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="mb-8 animate-fade-in">
            ...
            {/* モバイル版：横書き */}
            <span className="block md:hidden text-5xl sm:text-6xl font-serif font-bold text-white drop-shadow-2xl">
              {title}
            </span>
          </h1>
          ...
          <div className="mb-12 animate-fade-in-delay-1">
            ...
            {/* モバイル版：横書き */}
            <p className="block md:hidden text-lg sm:text-xl font-serif text-white/90 drop-shadow-lg leading-relaxed">
              {subtitle}
            </p>
          </div>
          ...
          <div className="mb-16 animate-fade-in-delay-2">
            <div className="max-w-2xl mx-auto">
              {description.split('\n').map((line, index) => (
                <p
                  key={index}
                  className="text-base sm:text-lg text-white/80 drop-shadow-md leading-relaxed mb-3 last:mb-0"
                >
                  {line}
                </p>
              ))}
          ...
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
            <div className="flex flex-col items-center text-white/70">
              <span className="text-xs mb-2 font-medium">下にスクロール</span>
              <svg
                className="w-6 h-6"
                fill="none"
                stroke="currentColor"
      ...
      <div className="absolute inset-0 z-5 pointer-events-none">
        ...
        {/* 左右の装飾線 */}
        <div className="absolute top-1/4 left-8 w-px h-1/2 bg-gradient-to-b from-transparent via-white/30 to-transparent hidden lg:block" />
        <div className="absolute top-1/4 right-8 w-px h-1/2 bg-gradient-to-b from-transparent via-white/30 to-transparent hidden lg:block" />
      </div>
    </section>
  )
}
...
export const HeroButton = ({
  ...
  const baseClasses = "inline-flex items-center px-8 py-4 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2"
